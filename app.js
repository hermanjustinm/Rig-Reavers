  ...
  manualScavengeList.innerHTML = manualScavengeAreas
    .map((area) => {
      ...
      <div class="manual-scavenge__item">
        ...
        <button class="primary" ${unlocked && canAffordEnergy && !state.scavenging.inProgress && !state.rest.inProgress ? "" : "disabled"}>
...
const updateWork = () => {
  ...
  if (state.work.inProgress) {
    ...
    workButton.disabled = !state.work.key || tierCooldown > 0 || state.rest.inProgress;
...
const startRest = () => {
  ...
  if (state.scavenging.inProgress || state.work.inProgress || state.training.inProgress || state.exploration.inProgress) {
    return;
  }
...
const startScavenge = (area) => {
  const unlocked = area.unlock ? area.unlock() : true;
  if (!unlocked || state.energy < area.cost || state.scavenging.inProgress || state.rest.inProgress) {
    return;
  }
...
const startWorkContract = () => {
  if (state.work.inProgress || !state.work.key || state.rest.inProgress) {
    return;
  }
...
const startExploration = (expedition) => {
  if (state.exploration.inProgress || !expedition.unlock() || state.rest.inProgress) {
    return;
  }
...
const startTraining = (key, cost, duration) => {
  if (state.training.inProgress || state.energy < cost || state.rest.inProgress) {
    return;
  }
  state.energy -= cost;
